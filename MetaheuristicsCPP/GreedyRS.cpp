...
CGreedyRS::CGreedyRS(IEvaluation<bool>& cEvaluation, IStopCondition& cStopCondition, mt19937& cRandomEngine, bool bShuffle, size_t iRepetitions = 1)
	: CSamplingOptimizer<bool>(cEvaluation, cStopCondition, c_random_generation), c_random_generation(cEvaluation.cGetConstraint(), cRandomEngine), c_greed_algorithm(cEvaluation, bShuffle), iRepetitions(iRepetitions)
{
...
bool CGreedyRS::b_run_iteration(long long iIterationNumber, clock_t tStartTime)
{
	...
	for (size_t i = 0; i < iRepetitions; ++i) {
